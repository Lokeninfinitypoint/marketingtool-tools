...
if (is_array($posts)): ?>
    <div class="pxl-recent-news pxl-recent-news1 <?php echo esc_attr($settings['pxl_animate']); ?>" data-wow-delay="<?php echo esc_attr($settings['pxl_animate_delay']); ?>ms" >
                            ...
                            <?php if($show_author == 'true'): ?>
                                <div class="pxl-post--author ">
                                    <a href="<?php echo esc_url(get_author_posts_url($post->post_author, $author->user_nicename)); ?>">
                                        <i class="caseicon-user  text-gradient"></i><?php echo esc_html__('By', 'brighthub').' '.esc_html($author->display_name); ?>
                                    </a>
                                </div>
                            <?php endif; ?>
                            <?php if($show_comment == 'true'): ?>
                                <div class="pxl-post--comment">
                                    <a href="<?php echo esc_url(get_permalink( $post->ID )); ?>#comments">
                                        <i class="caseicon-chat  text-gradient"></i>
                                        <?php echo esc_attr($comment_count ); ?>
                                        <?php if($comment_count > 1) { echo esc_html__('Comments', 'brighthub'); } ?>
                                        <?php if($comment_count == 1 || $comment_count == 0) { echo esc_html__('Comment', 'brighthub'); } ?>
                                    </a>
                                </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="pxl-post--readmore">
                            <a class="btn btn-text-parallax btn-icon-box" href="<?php echo esc_url(get_permalink( $post->ID )); ?>">
                                <span class="pxl--btn-text"><?php if(!empty($button_text)) {
                                    ...
                                    echo esc_html__('Read More', 'brighthub');
                                } ?></span>
                                <span class="pxl--btn-icon"><i class="caseicon-up-right-arrow"></i></span>
                            </a>
                        </div>
                    <?php endif; ?>
                </div>
