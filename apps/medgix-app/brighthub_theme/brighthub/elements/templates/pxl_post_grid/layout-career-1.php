...
$post_type = $widget->get_setting('post_type','career');
$layout = $widget->get_setting('layout_'.$post_type, 'career-1');

$show_location = $widget->get_setting('show_location');
$show_type= $widget->get_setting('show_type');
$show_slot= $widget->get_setting('show_slot');
...
$wrap_attrs = [
    'id'               => $html_id,
    'class'            => trim('pxl-grid pxl-grid__career-1'),
...
if( count($posts) <= 0){
    echo '<div class="pxl-grid__empty">'.esc_html__( 'No Career Found', 'brighthub' ). '</div>';
    return;
} ?>
