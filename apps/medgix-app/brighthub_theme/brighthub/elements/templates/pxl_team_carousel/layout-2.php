...
$reverseDirection = $widget->get_setting('reverseDirection', false);  
$opts = [
...
<?php if(isset($settings['team']) && !empty($settings['team']) && count($settings['team'])): 
    $image_size = !empty($settings['img_size']) ? $settings['img_size'] : '208x208'; ?>
    <div class="pxl-swiper-slider pxl-team pxl-team__style-2" <?php if($drag !== false) : ?>data-cursor-drag="<?php echo esc_attr__('DRAG', 'brighthub'); ?>"<?php endif; ?>>
            ...
            <div <?php pxl_print_html($widget->get_render_attribute_string( 'carousel' )); ?>>
                <div class="pxl-swiper-slider__wrapper">
                    <?php foreach ($settings['team'] as $key => $value):
                        ...
                        <div class="pxl-swiper-slider__item pxl-team__item">
                            <div class="pxl-team__item-inner <?php echo esc_attr($settings['pxl_animate']); ?>" data-wow-delay="<?php echo esc_attr($settings['pxl_animate_delay']); ?>ms" >
                                <?php if(!empty($image['id'])) { 
                                    ...
                                    <div class="pxl-team__item-image">
                                        <?php if ( ! empty( $value['item_link']['url'] ) ) { ?><a <?php echo implode( ' ', [ $link_attributes ] ); ?>><?php } ?>
                                            <?php echo wp_kses_post($thumbnail); ?>
                                        <?php if ( ! empty( $value['item_link']['url'] ) ) { ?></a><?php } ?>
                                    </div>
                                <?php } ?>
                                <div class="pxl-team__item-content">
                                    <h3 class="pxl-team__item-title"><?php echo pxl_print_html($title); ?></h3>
                                    <div class="pxl-team__item-pos"><span><?php echo pxl_print_html($position); ?></span></div>
                                </div>
                           </div>
                        </div>
                    <?php endforeach; ?>
