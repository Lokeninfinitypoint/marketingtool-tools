<?php
/**
* The BrightHub_Admin initiate the theme admin
*/
...
class BrightHub_Admin extends BrightHub_Base{
	...
	public function enqueue() {
		...
		if ( in_array( $pagenow, array( 'edit-tags.php', 'term.php' ) ) ) {
			$dependencies = array( 'jquery', 'wp-lists', 'admin-tags', 'wp-ajax-response' );
		}
		
		wp_enqueue_script( 'pxlart-admin', get_template_directory_uri() . '/inc/admin/assets/js/admin.js', $dependencies, false, true );
    ...
    public function fix_parent_menu() {
		...
		$submenu['pxlart'][0][0] = brighthub()->get_name().' '.esc_html__( 'Dashboard', 'brighthub' );
