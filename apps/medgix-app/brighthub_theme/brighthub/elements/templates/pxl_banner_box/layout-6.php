<?php
    if ( ! empty( $settings['scl6_link']['url'] ) ) {
        $widget->add_render_attribute( 'scl6_link', 'href', $settings['scl6_link']['url'] );
    
        if ( $settings['scl6_link']['is_external'] ) {
            $widget->add_render_attribute( 'scl6_link', 'target', '_blank' );
        }
    
        if ( $settings['scl6_link']['nofollow'] ) {
            $widget->add_render_attribute( 'scl6_link', 'rel', 'nofollow' );
        }
    }
?>
<div class="pxl-banner-box pxl-banner-box__style-6">
    <?php if ( ! empty( $settings['scl6_link']['url'] ) ) { ?><a <?php pxl_print_html($widget->get_render_attribute_string( 'scl6_link' )); ?> class="pxl-banner-box__link"><?php } ?> 
    <?php if ( ! empty( $settings['scl6_link']['url'] ) ) { ?></a><?php } ?>
    <div class="pxl-banner-box__image">
        <div class="pxl-banner-box__image-1">
            <?php $image_size = 'full';
                if(!empty($settings['scl6_img_1']['id'])) : $img_id = $settings['scl6_img_1']['id']; endif;
        ...
        <div class="pxl-banner-box__image-2">
            <?php $image_size = 'full';
                if(!empty($settings['scl6_img_2']['id'])) : $img_id = $settings['scl6_img_2']['id']; endif;
                $img  = pxl_get_image_by_size( array(
                    'attach_id'  => $img_id,
                    'thumb_size' => $image_size,
                    'class' => ''
        ...
        <div class="pxl-banner-box__image-3">
            <?php $image_size = 'full';
                if(!empty($settings['scl6_img_3']['id'])) : $img_id = $settings['scl6_img_3']['id']; endif;
                $img  = pxl_get_image_by_size( array(
                    'attach_id'  => $img_id,
                    'thumb_size' => $image_size,
                    'class' => ''
    ...
    <div class="pxl-banner-box__meta">
        <div class="pxl-banner-box__title">
            <span class="pxl-banner-box__title-icon">
                <?php if (!empty($settings['scl6_icon']['value']) ) : ?>
                    <?php \Elementor\Icons_Manager::render_icon( $settings['scl6_icon'], [ 'aria-hidden' => 'true', 'class' => '' ], 'i' ); ?>
                <?php endif; ?>
            </span>
            <span class="pxl-banner-box__title-text">
                <?php echo esc_html($settings['scl6_title']); ?>
            </span>
        </div>
